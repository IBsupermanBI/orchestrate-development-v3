...
function credits(r) { const rates=card.rates[r.observed_model]; const u=r.token_usage, date=r.recorded_at.slice(0,10); if(!rates||!u||date<card.verified_at||(r.observed_model==='gpt-5.6-sol'&&date>'2026-11-21')) return null;
 ...
 const speed=r.service_tier||r.speed; let multiplier=1; if(speed && !['standard','default'].includes(speed)) {if(r.observed_model==='gpt-6-astra'&&['fast','priority'].includes(speed)) multiplier=2.5; else return null;}
...
function transcriptFallback(rows, home, session) {
 ...
 function walk(dir) { if(!fs.existsSync(dir))return;for(const e of fs.readdirSync(dir,{withFileTypes:true})) {const f=path.join(dir,e.name);if(e.isDirectory())walk(f);else if(e.isFile()&&e.name.endsWith('.jsonl')) {const id=[...ids].find(id=>e.name.endsWith(id+'.jsonl'));if(!id)continue;if(fs.statSync(f).size>64*1024*1024){notes.push('Transcript exceeds 64 MiB: '+id);continue;}let meta=null,latest=null;const models=new Set();let turn=null;const starts=new Map();
 ...
 if(['task_complete','task_completed','turn_aborted'].includes(p.type)){const tid=p.turn_id||turn;if(!rows.some(x=>x.session_id===id&&x.turn_id===tid&&['turn_completed','turn_interrupted'].includes(x.event)))additions.push({session_id:id,turn_id:tid,event:'turn_completed',recorded_at:r.timestamp,started_at:starts.get(tid),project_root:meta?.cwd,interaction_class:'UNKNOWN'});starts.delete(tid);}
 ...
 if(latest&&!rows.some(x=>x.session_id===id&&x.token_usage))additions.push({session_id:id,event:'usage_snapshot',recorded_at:latest.timestamp,project_root:meta.cwd,observed_model:models.size===1?[...models][0]:null,usage_scope:'transcript_session',usage_kind:'cumulative_snapshot',usage_additive:false,token_usage:latest.usage});
 for(const [tid,at]of starts)if(!rows.some(x=>x.session_id===id&&x.turn_id===tid&&x.event==='turn_started'))additions.push({session_id:id,turn_id:tid,event:'turn_started',recorded_at:at,project_root:meta.cwd});
...
function build(all,opt={}) {
 ...
 function agent(r) { const id=actor(r); if(!agents.has(id)) agents.set(id,{id,session_id:r.session_id||null,parent_id:r.parent_agent_id||r.parent_session_id||null,models:[],profiles:[],intervals:[],open_blocks:0}); const a=agents.get(id); for(const [k,v] of [['models',r.observed_model],['profiles',r.profile]]) if(v&&!a[k].includes(v)) a[k].push(v); return a; }
 for(const r of rows) {
  const k=key(r), t=Date.parse(r.recorded_at);
  if(['turn_started','subagent_started'].includes(r.event)) starts.set(k,r);
  if(['turn_completed','turn_interrupted','subagent_completed'].includes(r.event)) {
   const begun=starts.get(k), finish=Date.parse(r.ended_at||r.recorded_at), rawStart=Date.parse(r.started_at||begun?.started_at||begun?.recorded_at);
 ...
 for(const r of [...usage.values()].sort((a,b)=>Date.parse(a.recorded_at)-Date.parse(b.recorded_at))) {
  ...
  const groupKey=JSON.stringify([actor(r),r.observed_model||'unknown',r.usage_scope||'unknown',additive?'delta':r.usage_kind||'unknown']);
  if(!usageGroups.has(groupKey)) usageGroups.set(groupKey,{agent_id:actor(r),model:r.observed_model||'unknown',scope:r.usage_scope||'unknown',kind:additive?'additive_deltas':r.usage_kind||'unknown',records:0,tokens:{input_tokens:0,cached_input_tokens:0,output_tokens:0,reasoning_output_tokens:0},estimated_credits:0,unpriced_records:0});
...
function main(args) {
 if(args.includes('--help')) {console.log('timesheet.cjs report|save|current --project PATH [--days N | --from ISO --to ISO] [--session ID] [--model ID] [--workflow NAME] [--home PATH] [--out FILE]');return;}
 const mode=args.shift()||'report',opt={mode};if(!['report','save','current'].includes(mode))throw Error('Unknown mode');
 for(let i=0;i<args.length;i+=2) {const k=args[i].replace(/^--/,'');if(!['project','days','from','to','session','model','workflow','home','out'].includes(k)||!args[i+1]||args[i+1].startsWith('--'))throw Error('Invalid option '+args[i]);opt[k]=args[i+1];}
